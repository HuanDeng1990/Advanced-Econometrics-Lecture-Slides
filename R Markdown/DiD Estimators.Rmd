...
```
### Understand the data & visualize policy rollout

```{r, message=FALSE, warning=FALSE}
# This section standardizes the key columns and produces quick visuals
# tailored to this dataset (state-year panel with Medicaid expansion timing).

suppressPackageStartupMessages({
  library(dplyr); library(tidyr); library(ggplot2); library(rlang); library(knitr)
})

# 1) Map to standardized names for clarity
df_here <- df
cols <- list(
  id          = "stfips",   # state FIPS
  time        = "year",
  y           = "dins",     # outcome used in the exercise
  first_treat = "yexp2",    # expansion year; NA implies "never (in sample window)"
  D           = NULL        # not needed because we have first_treat
)

id_sym  <- sym(cols$id);  time_sym <- sym(cols$time);  y_sym <- sym(cols$y)
ft_sym  <- sym(cols$first_treat)

derive_first_treat <- function(df, id, time, D) {
  df %>% arrange({{id}}, {{time}}) %>% group_by({{id}}) %>%
    mutate(first.treat = dplyr::if_else(all({{D}} == 0 | is.na({{D}})), 0L,
                                        suppressWarnings(min({{time}}[{{D}} == 1], na.rm = TRUE)) %>% as.integer())) %>%
    ungroup()
}

# Standardize columns: id, time, y, first.treat, D
df_std <- df_here %>%
  rename(id = !!id_sym, time = !!time_sym, y = !!y_sym, first.treat = !!ft_sym) %>%
  mutate(first.treat = ifelse(is.na(first.treat) | first.treat == 0, 0L, as.integer(first.treat)),
         D = ifelse(first.treat == 0L, 0L, as.integer(time >= first.treat))) %>%
  arrange(id, time) %>%
  mutate(across(c(id, time), as.integer))

# Event time (NA for never-treated); status for plots
df_std <- df_std %>%
  mutate(event_time = ifelse(first.treat == 0L, NA_integer_, time - first.treat),
         status = ifelse(D == 1, "treated", "untreated"))

# 2) Quick overview
n_obs  <- nrow(df_std); n_id <- dplyr::n_distinct(df_std$id); n_time <- dplyr::n_distinct(df_std$time)
dup_ct <- df_std %>% count(id, time) %>% filter(n > 1) %>% nrow()
cat("Rows:", n_obs, " | States:", n_id, " | Years:", n_time, " | Duplicate (id,time) rows:", dup_ct, "\\n")
knitr::kable(head(df_std, 6), caption = "First 6 standardized rows (id, time, y, first.treat, D)")

# 3) Rollout visuals

# (a) Adoption matrix (treated vs untreated)
ggplot(df_std, aes(x = time, y = factor(id))) +
  geom_tile(aes(fill = status), color = "white") +
  scale_fill_manual(values = c("treated" = "steelblue", "untreated" = "grey92")) +
  labs(x = "Year", y = "State (FIPS)", fill = "", title = "Medicaid expansion rollout: adoption matrix") +
  theme_minimal(base_size = 11)

# (b) Event-time heatmap (k = t − g)
ggplot(df_std, aes(x = time, y = factor(id))) +
  geom_tile(aes(fill = event_time), color = "white") +
  scale_fill_gradient2(low = "firebrick", mid = "white", high = "steelblue",
                       midpoint = 0, na.value = "grey92") +
  labs(x = "Year", y = "State (FIPS)", fill = "k = t − g",
       title = "Event-time map: pre (<0), at 0, and post (>0)") +
  theme_minimal(base_size = 11)

# (c) Cohort sizes & cumulative expansion
coh <- df_std %>% distinct(id, first.treat) %>% filter(first.treat > 0)
p_hist <- ggplot(coh, aes(first.treat)) +
  geom_histogram(binwidth = 1, boundary = 0, color = "white") +
  labs(x = "Expansion year (g)", y = "States", title = "Cohort sizes (treated only)") +
  theme_minimal(base_size = 11)

cum_adopt <- coh %>% count(first.treat) %>% arrange(first.treat) %>%
  mutate(cum = cumsum(n) / sum(n))
p_cum <- ggplot(cum_adopt, aes(first.treat, cum)) +
  geom_line() + geom_point() +
  labs(x = "Year", y = "Cumulative share treated", title = "Cumulative Medicaid expansion rollout") +
  theme_minimal(base_size = 11)
